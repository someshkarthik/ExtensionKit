...
public extension UIView {
    ...
    func unblink(duration: TimeInterval = 0.5, delay: TimeInterval = 0.0, alpha: CGFloat = 1.0) {
        UIView.animate(withDuration: duration, delay: delay, options: [.curveEaseIn,.beginFromCurrentState,.allowUserInteraction], animations: {
            self.alpha = alpha
        })
    }
}
